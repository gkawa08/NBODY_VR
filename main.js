...
let velScaleMultiplier = 1.0, spinScaleMultiplier = 1.0;
...
const TRUE_KICK_COLOR = 0xffff00;
...
function handleMergeEvent(time, bhData, com) {
    ...
    if (time < et) {
        ...
        if (rd && b1 && b2) {
            ...
            if (time === timeKeys[kickIdx]) drawArrow(new THREE.Vector3(rd.x - com.x, rd.y - com.y, rd.z - com.z), vKick, vKick.length() * 20.0 * velScaleMultiplier, TRUE_KICK_COLOR);
...
function setupUI() {
    ...
    ['ns-visible-checkbox', 'com-frame-checkbox', 'camera-track-checkbox'].forEach(id => document.getElementById(id).addEventListener('change', e => {
